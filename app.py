...
if api_key:
    ...
    if st.button("올리 그려줘!"):
        if user_input:
            with st.spinner("올리가 그림을 완성하는 중..."):
                try:
                    # [해결 1] 할당량 아끼기: 번역 실패 시에도 그림은 나오도록 기본값 설정
                    ...
                    try:
                        ...
                        pass # 할당량 오류 시 기본 영어 문구 사용

                    # [해결 2] 엑박 방지: 올리 특징 고정 및 URL 안전 변환
                    ...
                    # [핵심 필살기] 한글과 공백을 인터넷 표준 주소로 강제 변환합니다. (이게 없어서 그동안 엑박이 떴던 겁니다!)
                    ...
                    st.error("이미지 서버가 잠시 바빠요. 다시 한번 버튼을 눌러주세요!")
        else:
            st.warning("무엇을 그릴지 알려주세요!")
else:
    st.warning("왼쪽 사이드바에서 API Key를 입력해야 올리가 그림을 그릴 수 있어요!")
